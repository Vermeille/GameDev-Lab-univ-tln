...
def _press_key(e):
    #print('PRESS', e)
    if e.keycode in bite:
        __canv.after_cancel(bite[e.keycode])
        del bite[e.keycode]
    else:
        print('PRESS', e)
    __events.add(e.keycode)

def _rel_key(e):
    print('RELEASE', e)
    bite[e.keycode] = __canv.after_idle(lambda: print('PRESS', e))
    __events.remove(e.keycode)
...
def init_window(pnom='', pla=None, pha=None, color='white'):
    ...
    __canv.bind('<KeyPress>', _press_key)
    __canv.bind('<KeyRelease>', _rel_key)
    __canv.focus_set()
    __canv.pack()
